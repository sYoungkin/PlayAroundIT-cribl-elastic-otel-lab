...
cp "$CERT_SRC/kibana.crt" "$CERT_DST/kibana.crt"
cp "$CERT_SRC/kibana.key" "$CERT_DST/kibana.key"
chown -R root:kibana "$CERT_DST"
chmod 750 "$CERT_DST"
chmod 640 "$CERT_DST"/*
rm -f "$CERT_SRC/ca.crt" "$CERT_SRC/kibana.crt" "$CERT_SRC/kibana.key"
...
server.ssl.certificate: ${CERT_DST}/kibana.crt
...
log ""
log "NEXT (manual bootstrap):"
log "  1. Get elastic-1's IP"
log "  2. sudo sed -i 's/ELASTIC_HOST/<elastic-1-ip>/' ${KIBANA_CONF}"
log "  3. sudo systemctl restart kibana"